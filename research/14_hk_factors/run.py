#!/usr/bin/env python
"""HK equity factors — run 4 reference signals on 12 HK large caps."""
...
def main():
    ...
    print(f"[data] loading {len(cfg['universe'])} HK tickers via yfinance ...")
    rets = load_returns_panel(
        tickers=list(cfg["universe"]),
        start=cfg["start_date"], end=cfg["end_date"],
        cache_name=f"hk_{cfg['start_date']}_{cfg['end_date']}",
    )
    ...
    results = {
        "config": {"n_months": int(rets.shape[0]), "n_assets": int(rets.shape[1])},
        "signals": {},
    }
    ...
    for name in cfg["signals"]:
        ...
        print(f"{name:<8} {full['sharpe']:>+10.3f} {iso['is']['sharpe']:>+10.3f} "
