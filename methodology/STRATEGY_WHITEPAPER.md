...
**Version**: 2.0 (statistically audited)
**Date**: April 2026
...
| Execution | Alpaca Trading API v2 | Real-time | Real-time |
...
[T+1 Market Open]
  7. Close existing positions (Alpaca API)
  8. Submit new orders (market orders, DAY TIF)
  9. Reconcile target vs actual
  10. Archive signals + reconciliation to JSON
```

### 11.2 Alpaca Configuration

| Parameter | Value |
|-----------|-------|
| Account | Paper trading ($100K) |
| API version | v2 (alpaca-py SDK) |
| Order type | Market |
| Time in force | DAY |
| Shorting | Enabled |

### 11.3 Command Interface

```bash
python alpaca_live.py --status    # Account + positions
python alpaca_live.py --signal    # Generate signals only
python alpaca_live.py --execute   # Generate + trade
python alpaca_live.py --close     # Liquidate all
```
...
*Institutional-grade statistical rigor applied throughout*
